...
TNC.events = {
  ...
  onMapMove: function (map, ractive, markerLayerGroup, turfOverlayLayerGroup){
    // map.on('move', function () {
    //   if (ractive.get('appstate.activeTab' === 'button-summary')) {
  ...
  resetFilter: function (map, ractive, markerLayerGroup, turfOverlayLayerGroup) {
  ...
  updateFilterState: function (map, ractive, markerLayerGroup, turfOverlayLayerGroup) {
    var dataqueryFilters = 'appstate.filters.dataquery';
    var turfFilters = 'appstate.filters.turf';    

    // for (var key in ractive.get(turfFilters)) {
    //   getSelectChange(key, turfFilters);          
    // }

    getSpatialSelectChange('spatial-select-options', turfFilters);

    //getSpatialSelectChange('spatial-select-options');
    ...
    // function buildSpatialQueryLoop() {
    // 	var spatialFilters = ractive.get('appstate.filters.turf');
    // 	var singleFilters = [];
    // 	for (var key in spatialFilters) {
    // 		if (spatialFilters[key]) {
    // 			var fieldName = TNC.config.filters.spatial[key].cartoDbField;
    // 			for (var i = 0; i < spatialFilters[key].length; i++) {
    // 				singleFilters.push("'" + spatialFilters[key][i] + "'");javascript:void(0)
    // 			}
    // 			console.log(singleFilters);
    // 			var stringJoin = singleFilters.join(',');
    // 			console.log(stringJoin);
    // 			var cartoDbQuery = "http://dmajka.cartodb.com/api/v2/sql?format=GeoJSON&q=SELECT * FROM " + key + " WHERE " + fieldName + " IN ("+stringJoin+")";
    // 			console.log(cartoDbQuery);
    // 			return cartoDbQuery;   			
    // 		}
    // 	}
    // }
  ...
  getSpatialExtentLayer: function (selectID, optionsID) {
  ...
  showFilterModal: function () {
    ractive.on('filter-pop', function (event) {
  ...
  showDetailsFromPopup: function (map) {
  ...
  showPopup: function (markerLayerGroup) {
  ...
  mapHighlight: function (markerInfo, markerLayerGroup) {
  ...
  popit: function(layer) {
  	console.log('popit!');
  	console.log(layer);
  	layer.openPopup;
  },

  showFootprintOnClick (map, projectOverlayGroup) {
  ...
  popupOnScroll: function (markerLayerGroup) {
